	...
	public void run() {
		...
		for (Nation nation : DataHandler.getNations().values()) {
			...
			for(Zone zone : nation.getZones().values()) {
				...
				if(!owner.isPresent()) continue; //probably something went wrong //todo gotta recheck if it only gets online players
			...
			//not for each zone or else it'll save every time
